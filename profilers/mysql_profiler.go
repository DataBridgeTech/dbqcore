...
func (p *MysqlDbqDataProfiler) GetColumns(ctx context.Context, schemaName string, tableName string) ([]*dbqcore.ColumnInfo, error) {
	...
	for rows.Next() {
		...
		cols = append(cols, &dbqcore.ColumnInfo{Name: colName, Type: colType, Comment: comment, Position: pos})
	}
...
func (p *MysqlDbqDataProfiler) GetMostFrequentValue(ctx context.Context, dataset string, column *dbqcore.ColumnInfo) (*string, error) {
	mfvQuery := fmt.Sprintf(`
			SELECT col FROM (
				SELECT %s as col, count(*) as c FROM %s GROUP BY col ORDER BY c DESC LIMIT 1
			) as t`, column.Name, dataset)
...
func (p *MysqlDbqDataProfiler) IsStringType(dataType string) bool {
	...
	return strings.Contains(dataType, "char") ||
		strings.Contains(dataType, "varchar") ||
